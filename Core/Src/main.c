...
static void Set_LED_Duty(uint8_t percent);
/* USER CODE END PFP */
...
/**
  * @brief  ADC DMA Half Transfer callback
  */
...
/**
  * @brief  EXTI line detection callback (user button PA0)
  */
...
/**
  * @brief  Start periodic ADC logging (TIM2 + ADC1 + DMA)
  */
static void Start_Logging(void)
{
  // Clear flags
...
/**
  * @brief  Stop periodic ADC logging
  */
static void Stop_Logging(void)
{
...
/**
  * @brief  Set LED brightness (0–100%) on PD12 via TIM4_CH1
  */
static void Set_LED_Duty(uint8_t percent)
{
...
int main(void)
{
  ...
  // Start PWM on LED (PD12 / TIM4_CH1)
  HAL_TIM_PWM_Start(&htim4, TIM_CHANNEL_1);
  Set_LED_Duty(10);   // idle dim LED
  ...
  while (1)
  {
    // Set LED according to logging state
    if (logging_enabled)
    {
      Set_LED_Duty(80);   // bright when logging
    }
    else
    {
      Set_LED_Duty(10);   // dim when idle
    }

    // Process blocks when DMA signals them ready
    if (adc_half_ready)
    {
      adc_half_ready = 0;
      LOGGER_SendBlock(&huart2, &adc_buffer[0], ADC_BUF_SIZE / 2);
    }
...
static void MX_GPIO_Init(void)
{
  ...
  HAL_GPIO_WritePin(GPIOC, GPIO_PIN_0, GPIO_PIN_RESET); // OTG_FS_PowerSwitchOn (unused)

  /*Configure GPIO pin Output Level */
  HAL_GPIO_WritePin(GPIOD, GPIO_PIN_4, GPIO_PIN_RESET); // Audio_RST (unused)
  ...
  /*Configure PD12 as TIM4_CH1 (already configured by HAL_TIM_MspPostInit) */

  /*Configure PA0 as EXTI (User Button) */
